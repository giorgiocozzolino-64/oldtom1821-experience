export const bottles = Array.from({ length: 60 }, (_, index) => {
  const number = index + 1
  const serial = `FCA26-LE-${String(number).padStart(3, "0")}`

  return {
    serial,
    bottleNumber: `${String(number).padStart(2, "0")} OF 60`,
...
The cocktail follows the classic Italian equal-parts structure: gin, bitter aperitivo and sweet vermouth.
...
The result was a perfectly balanced cocktail: bold, bitter, elegant and timeless.
